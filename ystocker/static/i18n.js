...
const I18n = (() => {
  ...
  const LANGS = {
    ...
    'mult.fwd_scope':     { en: 'Our own reading, computed here from index constituents and recorded once a day, so this series starts at the first snapshot. For history on each index — years of it for the S&P 500 — see the published consensus chart below.',
                            zh: '本站自行计算的数值：由指数成分股算出并每日记录一次，因此该序列自首次快照开始。各指数的历史数据（标普500有多年）请见下方的公开一致预期图表。' },
    ...
    'mult.consensus_title': { en: 'Consensus Forward P/E — published history',
                            zh: '一致预期市盈率 — 公开历史数据' },
    'mult.consensus_desc': { en: 'What analysts actually expected on each past date — the same question as the card above, answered by the firms that publish it, so it has real history. The S&P 500 line is FactSet\u2019s stated forward 12-month P/E, back to 2017.',
                            zh: '分析师在过去每一个日期上真实的预期——与上方卡片是同一个问题，但由公开发布这些数据的机构给出，因此拥有真实历史。标普500曲线取自 FactSet 公布的未来12个月预期市盈率，回溯至2017年。' },
