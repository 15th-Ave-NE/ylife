...
const I18n = (() => {
  ...
  const LANGS = {
    ...
    'mult.fwd_scope':     { en: 'Our own reading, computed here from index constituents and recorded once a day, so this series starts at the first snapshot. The chart below plots this same SPY series against FactSet’s published history, which is where to see whether today’s reading is high or low; QQQ stays here, because no free source measures the Nasdaq-100 on a comparable basis.',
                            zh: '本站自行计算的数值：由指数成分股算出并每日记录一次，因此该序列自首次快照开始。下方图表将这条 SPY 序列与 FactSet 公布的历史并列绘制，可用于判断今日读数处于高位还是低位；QQQ 仅保留在此处，因为没有任何免费来源以可比口径衡量纳斯达克100。' },
