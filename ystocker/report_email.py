...
# Where the send-once marker lives. Imported lazily inside functions rather than
# at module scope: agents.py imports this module, so a top-level import back into
# it would be a cycle.
_MARKER_SUFFIX = ".emailed"
...
_STR: dict[str, dict[str, str]] = {
    "en": {
        ...
        "degraded":  ("Part of this run was answered by a fallback model ({models}) "
                      "after the configured one hit its daily quota."),
        "recovered": ("This run failed before it could file its report; the analysis "
                      "below was recovered from the run's own stream."),
        "clipped":   ("This email shows the report up to here. The remaining sections "
                      "are on the site."),
        "why":       ("You are receiving this because you ran this analysis on "
                      "trade-agents. It is sent once, when the run finishes."),
        "foot":      ("Generated by trade-agents from the TradingAgents multi-agent "
                      "framework. For research only — not investment advice."),
    },
    "zh": {
        ...
        "open":      "打开完整报告",
        "elapsed":   "耗时 {mins}",
        "degraded":  "本次运行的部分内容由备用模型（{models}）生成，因配置的模型已用完当日配额。",
        "recovered": "本次运行在生成报告前失败；以下分析是从运行流中恢复的。",
        "clipped":   "邮件仅显示到此处，其余章节请在网站查看。",
        "why":       "您收到此邮件是因为您在 trade-agents 上运行了本次分析。仅在运行完成时发送一次。",
        "foot":      "由 trade-agents 基于 TradingAgents 多智能体框架生成。仅供研究参考，不构成投资建议。",
    },
}
...
def _body_rows(job: dict[str, Any], lang: str, link: str) -> str:
    ...
    reserve = min(len(decision_html), _HTML_BUDGET) if decision_html else 0
    ...
    for sec in sections:
        ...
        if dropped or (used + len(chunk) + reserve > _HTML_BUDGET and rows):
            dropped += 1
            continue
        rows.append(chunk)
        used += len(chunk)
...
def build(job: dict[str, Any], link_base: str = "") -> Optional[tuple[str, str, str]]:
    ...
    day = str(job.get("date") or "")
    link = f"{(link_base or base_url()).rstrip('/')}/agents?job={job.get('id')}"

    verdict = (job.get("decision") or "").strip().splitlines()
    short = verdict[0].strip()[:40] if verdict and verdict[0].strip() else ""
    subject = _t(lang, "subject").format(
        ticker=ticker, decision=f" — {short}" if short else "", date=day)

    rows = _body_rows(job, lang, link)
    if not rows:
        return None
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:{_BG};padding:28px 12px">
    <tr><td align="center">
      <table width="760" cellpadding="0" cellspacing="0"
        ...
        <tr><td style="background:linear-gradient(135deg,#1d4ed8,#7c3aed);padding:24px 28px">
          <h1 style="margin:0;font-size:20px;font-weight:700">
            <a href="{_esc(link)}" style="color:#fff;text-decoration:none">trade-agents</a>
          </h1>
          <p style="margin:6px 0 0;color:#bfdbfe;font-size:15px">
            {_esc(ticker)} — {_esc(_t(lang, 'header'))}</p>
          <p style="margin:3px 0 0;color:#93c5fd;font-size:12px">{_esc(meta)}</p>
        </td></tr>
        {chip_row}
    ...
    head_lines = [f"{ticker} — {_t(lang, 'header')}", meta]
    if short:
        head_lines.append(f"{_t(lang, 'decision')}: {short}")
    head_lines.append(f"{_t(lang, 'open')}: {link}")
    text = "\n".join([
...
def _marker(job_id: str):
    ...
    return JOB_DIR / f"{job_id}{_MARKER_SUFFIX}"
...
def send(job: dict[str, Any]) -> bool:
    """Mail the report for ``job``. Assumes the claim is already held."""
    addr = _recipient(job)
    ses_from = (os.environ.get("SES_FROM_EMAIL") or "").strip()
    if not addr or not ses_from:
        return False
    ...
    try:
        ...
        # production-access grant are per region, and a verified domain in one
        # is not verified in another.
        ...
        log.warning("report_email: SES send failed for %s -> %s: %s",
                    job.get("id"), addr, exc)
        return False
    log.info("report_email: sent %s (%s, %d bytes html) to %s",
             job.get("id"), job.get("ticker"), len(html), addr)
    return True
